...
def ingest_laws(batch_size: int = 100):
    ...
    """
    logger.info("=============================================================")
    logger.info("  ChromaDB Japanese Law Ingest Pipeline - Compliance RAG     ")
    logger.info("=============================================================")
    
    # 1. 법령 XML 파일 검사 및 자동 다운로드
    xml_files = list(LAWS_DIR.glob("*.xml"))
    if not xml_files:
        logger.error("[!] No law XML files found in data/laws/. Starting automatic download...")
        download_all_targets()
        xml_files = list(LAWS_DIR.glob("*.xml"))
        
    if not xml_files:
        logger.error("[x] Error: No law XML files could be found or downloaded. Aborting ingestion.")
        return False
        
    logger.info(f"[+] Found {len(xml_files)} law XML files to process.")
    ...
    for xml_path in xml_files:
        ...
        try:
            ...
            logger.info(f"    -> Parsed {len(chunks)} articles from '{parser.law_title}'")
    ...
    for json_path in json_files:
        ...
        try:
            ...
            logger.info(f"    -> Parsed {len(data)} items from '{json_path.name}'")
        except Exception as e:
    ...
    try:
        ...
        for pdf_path in pdf_files:
            ...
            try:
                ...
                logger.info(f"    -> Parsed {len(paragraphs)} chunks from '{pdf_path.name}'")
            except Exception as e:
                logger.error(f"    [-] Error parsing {pdf_path.name}: {e}")
    except ImportError:
        logger.error("[!] PyMuPDF (fitz) not installed. Skipping PDF parsing.")
            
    total_chunks = len(all_chunks)
    if total_chunks == 0:
        logger.error("[x] Error: No articles parsed. Aborting ingestion.")
        return False
    ...
    # 4. 배치(Batch) 단위 적재
    logger.info(f"[*] Uploading to ChromaDB in batches (size: {batch_size})...")
    ...
    for i in range(0, total_chunks, batch_size):
        ...
        try:
            ...
            logger.info(f"    [Batch] Uploaded chunks {i + 1} to {min(i + batch_size, total_chunks)} / {total_chunks}")
        except Exception as e:
