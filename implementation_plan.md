...
1. **Stripe JPY 요금제 및 기능별 차등 크레딧 과금 모델 (대표님 긴급 지시사항 수용)**:
   * **구독/결제 상품 구성**:
     * **BIM Light (월 1,500엔)**: 30 Credits 제공.
     * **BIM Business (월 4,900엔)**: 100 Credits 제공.
     * **BIM Enterprise (월 9,800엔)**: 300 Credits 제공.
     * **Pay-per-case (건당 1,500엔)**: 비구독자용 1회성 결제 (30 Credits 제공).
   * **기능별 차등 크레딧 차감 구조 (Differentiated Credit Deduction)**:
     * 🟢 **PDF 도면 3D IFC 복구 및 Three.js 뷰어 시각화**: **건당 3 Credits** 차감.
     * 🔥 **일본 건축기준법 AI 적합성 검증 및 자가확인서 PDF 리포트 생성/다운로드**: **건당 10 Credits** 차감 (서버 RAG 연산 및 PDF 벡터 날인 합성 비용 보전).
...
3. **404 무장애성**: 사이드바에 존재하는 모든 메뉴(`Upload`, `Settings`, `Help`, `Billing`)를 클릭했을 때 더 이상 404 에러 화면이 나타나지 않고, 정밀 구현된 SaaS 콘솔 UI 또는 상용 안내 팝업이 활성화될 것.
4. **리포트 아카이빙**: 에디터에서 도면 보정 완료 후 생성된 준공 BIM 검격 PDF 문서가 아카이브 목록에 정상 등록되고, 재요청 시 서버 부하 없이 초고속 캐시 다운로드가 집행될 것.
5. **사이버네틱 HUD 극상 비주얼**: 중앙의 Three.js 뷰어의 3D 연출이 대표님의 스크린샷과 정확하게 싱크되어, 하단 6개 버튼을 클릭할 때마다 3D 건물과 네온 펜스가 완벽히 상호작용할 것.

---
🛡️ **코다리 개발부장 최종 수정 보고 (대표님! 미래형 HUD 테크 대시보드 비주얼 혁명 계획을 계획서에 반영 완료했습니다! 즉시 코드 타격에 착수하겠습니다! 충성!)**
